...
class TestingMain:
    ...
    def bicepCurls(self, filename_input): # THIS ONE IS FROM RHS
        ...
        # Initialize hand detector and timer data
        ...
        while True:
            ...
            if len(lmList) != 0:
                ...
                if per == 100:
                    color = (0, 255, 0)
                    if dir == 0 and timer_data['count_running']==True:
                        count += 0.5
                        dir = 1
                if per == 0:
                    color = (0, 0, 255)
                    if dir == 1 and timer_data['count_running']==True:
                        count += 0.5
                        dir = 0
            ...
            img = gesture_timer(hand_detector, img, timer_data)
    ...
    def preacherCurls(self,filename_input):
        ...
        while True:
           ...
           # img_resized = resize_image(img, width=800)  # Resize the frame
            ...
            if len(lmList) != 0:
                ...
                angle = detector.findAngle(img, 11, 13, 19, lineColor=lineColor)
                #Left Arm
    ...
    def shoulderpress(self, filename_input):
    # r is used because windows automatically puts backslashes when copying path, but we need forward slashes in python
        cap = cv2.VideoCapture(f"uploads/{filename_input}")
        per = 0 
        bar = 0
        ...
        self.performanceFile =  back_bend(timestamps, img)


    def hammerCurls(self,filename_input):
        # r is used because windows automatically puts backslashes when copying path, but we need forward slashes in python
        cap = cv2.VideoCapture(f"uploads/{filename_input}")
        per = 0 
        bar = 0
        ...
        output_file = r"static/videooutputs/convertedOutput.mp4"
        file_exists = os.path.exists(output_file)
        counting = 1
    ...
    def lateralRaise(self,filename_input):
        # r is used because windows automatically puts backslashes when copying path, but we need forward slashes in python
        cap = cv2.VideoCapture(f"uploads/{filename_input}")
        per = 0 
        bar = 0
        ...
        while True:
            ...
            if not success:
                break
            # img = cv2.imread("test.jpg") # Resize the frame
            img = detector.findPose(img, False)
            lmList = detector.findPosition(img, False)
            frame_height, frame_width = img.shape[:2]
            ...
            if len(lmList) != 0:
                ...
                bar = np.interp(angle, (30, 100), (100, 650))
                #print(per)
    ...
    def frontRaise(self,filename_input):
        # r is used because windows automatically puts backslashes when copying path, but we need forward slashes in python
        cap = cv2.VideoCapture(f"uploads/{filename_input}")
        per = 0 
        bar = 0
        ...
        while True:
            ...
            if len(lmList) != 0:
                #Right Arm 74 Being minimum and 159 being maximum
                angle = detector.findAngle(img, 24,12,16, lineColor=lineColor)
                shoudler_angle = detector.findAngle(img, 24, 12, 14, lineColor=lineColor)
                #Left Arm
                # angle = detector.findAngle(img, 11, 13, 15)
                per = np.interp(angle, (260, 350), (100,0))
                bar = np.interp(angle, (260, 350), (650, 100))
                #print(per)
            ...
            timestamps.append(shoulder_timestamp)
    ...
    def wristCurls(self,filename_input):
        # r is used because windows automatically puts backslashes when copying path, but we need forward slashes in python
        cap = cv2.VideoCapture(f"uploads/{filename_input}")
        per = 0 
        bar = 0
        ...
        while True:
            ...
            if len(lmList) != 0:
                #Right Arm 74 Being minimum and 159 being maximum
                angle = detector.findAngle(img, 14,16,20)
                #Left Arm
                # angle = detector.findAngle(img, 11, 13, 15)
    ...
    def pushup(self,filename_input):
    # For Importing Video {Devansh}
        cap = cv2.VideoCapture(f"uploads/{filename_input}")
        per = 0 
        bar = 0
        ...
        while True:
            ...
            if len(lmList) != 0:
                ...
                if knee_angle > 190 or knee_angle < 165:
    ...
    def chestdips(self,filename_input):
        per = 0 
        bar = 0
        color=0
        frame_height = 0
        ...
        while True:
            ...
            if len(lmList) != 0:
                ...
                bar = np.interp(angle, (198, 270), (100, 650))
                #print(per)
                color = (0, int(255 * (per / 100)), int(255 * (1 - per / 100)))
                # Check for the dumbbell curls
                if per == 100:
    ...
    def benchpress(self,filename_input):
        # For Importing Video {Devansh}
        cap = cv2.VideoCapture(f"uploads/{filename_input}")
        per = 0 
        bar = 0
    ...
    def bulgarianSplits(self,filename_input):
        # For Importing Video {Devansh}
        cap = cv2.VideoCapture(f"uploads/{filename_input}")
        per = 0 
        bar = 0
    ...
    def romaniandeadlifts(self, filename_input):
    ...
    def lunges(self,filename_input):
        # For Importing Video {Devansh}
        cap = cv2.VideoCapture(f"uploads/{filename_input}")
        per = 0 
        bar = 0
        ...
        while True:
            ...
            if len(lmList) != 0:
                # #Right Arm
                angle = detector.findAngle(img, 24, 26, 28, lineColor=lineColor )
                anotherfoot = detector.findAngle(img, 23, 25, 27, lineColor=lineColor)
                # knee_angle = detector.findAngle(img, 25, 23,11, lineColor=lineColor)
                backbend_angle = calculate_backbend_angle(lmList)
    ...
    def planks(self,filename_input):
        # For Importing Video {Devansh}
        cap = cv2.VideoCapture(f"uploads/{filename_input}")
        per = 0 
        bar = 0
    ...
    def legRaises(self,filename_input):
        # For Importing Video {Devansh}
        cap = cv2.VideoCapture(f"uploads/{filename_input}")
        per = 0 
        bar = 0
    ...
    def crunches(self,filename_input):
        # For Importing Video {Devansh}
        cap = cv2.VideoCapture(f"uploads/{filename_input}")
        per = 0 
        bar = 0
    ...
    def reverseCrunches(self,filename_input):
        # For Importing Video {Devansh}
        cap = cv2.VideoCapture(f"uploads/{filename_input}")
        per = 0 
        bar = 0
    ...
    def mountainClimbers(self,filename_input):
        cap = cv2.VideoCapture(f"uploads/{filename_input}")
        # For Importing Video {Devansh}
        per = 0 
        bar = 0
    ...
    def tricepropepushdown(self, filename_input):
        ...
        while True:
            ...
            if len(lmList) != 0:
                ...
                # Check for the exercise
                color = (0,255,0)
                if per == 100 and timer_data['count_running']==True:
                    color = (0,0,255)
                    if dir ==0:
                        count += 0.5
                        dir = 1
            ...
            if len(lmList) != 0:
                ...
                if per == 100:
                    ...
                    if dir ==0 and timer_data['count_running']==True:
                        count += 0.5
                        dir = 1
                if per == 0 and timer_data['count_running']==True:
                    color = (0,0,255)
                    if dir == 1:
                        count += 0.5
                        dir = 0
    ...
    def dumbbellRows(self, filename_input):
        ...
        while True:
            ...
            if len(lmList) != 0:
                angle = detector.findAngle(img, 12, 14, 16)
                knee_angle = detector.findAngle(img, 12, 24, 26, lineColor=lineColor)
    ...
    def bentOverRows(self,filename_input):
        # For Importing Video {Devansh}
        cap = cv2.VideoCapture(f"uploads/{filename_input}")
        per = 0 
        bar = 0
        ...
        while True:
            ...
            if len(lmList) != 0:
                ...
                timestamps.append(int(timestamp))
                 ...
                 #Left Arm
                per = np.interp(angle, (105,164 ), (100,0))
                bar = np.interp(angle, (105,164 ), (100, 650))
                color = (0, int(255 * (per / 100)), int(255 * (1 - per / 100)))
...
def is_ok_sign(thumb_tip, thumb_base, index_tip, index_base, middle_tip, ring_tip, pinky_tip):
    thumb_index_angle = calculate_angle(thumb_tip, thumb_base, index_tip)
    middle_curled = middle_tip[2] > index_base[2]
    ring_curled = ring_tip[2] > index_base[2]
    pinky_curled = pinky_tip[2] > index_base[2]
    return 70 <= thumb_index_angle <= 110 and middle_curled and ring_curled and pinky_curled

def gesture_timer(detector, img, timer_data):
    img = detector.findHands(img) # Process the image and set the results attribute
    ...
    if len(lmList) != 0:
        ...
        # Check if palm is open (index finger and pinky are higher than the thumb)
        if is_ok_sign(thumb_tip, thumb_base, index_tip, index_base, middle_tip, ring_tip, pinky_tip):
        # if index_tip[2] < thumb_tip[2] and pinky_tip[2] < thumb_tip[2]:
            if detector.isPalmFacingCamera(lmList) and not timer_data['count_running']:
                if not timer_data['timer_running']:
                    timer_data['start_time'] = time.time()
                    timer_data['timer_running'] = True
                    timer_data['count_running'] = True

        # if is_ok_sign(thumb_tip, thumb_base, index_tip, index_base, middle_tip, ring_tip, pinky_tip):
        if index_tip[2] < thumb_tip[2] and pinky_tip[2] < thumb_tip[2]:
            if detector.isPalmFacingCamera(lmList) and timer_data['count_running']:
                if timer_data['timer_running']:
                    elapsed_time = time.time() - timer_data['start_time']
                    timer_data['total_elapsed_time'] += elapsed_time
                    timer_data['timer_running'] = False
                    timer_data['count_running'] = False  # Stop counting reps when gesture is made             
    ...
    if timer_data['timer_running']:
        ...
        elapsed_time = time.time() - timer_data['start_time']
        current_time = timer_data['total_elapsed_time'] + elapsed_time
        cv2.putText(img, text+ f"{current_time:.2f} sec", text_position, font, scale, (255,255,255), thickness)
    else:
        frame_height, frame_width = img.shape[:2]
        lineColor = (0,0,255)
        text = "Timer stopped:"
        ...
        cv2.rectangle(overlay, (frame_width,100), (0,0), UI_Color, cv2.FILLED)
        cv2.putText(img, text+ f"{timer_data['total_elapsed_time']:.2f} sec", text_position, font, scale, (255,255,255), thickness)
